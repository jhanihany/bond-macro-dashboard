...
def get_wti():
    ...
    try:
        ...
        for ts, close in zip(timestamps, closes):
            if close is None:
                continue
            dt_et = datetime.fromtimestamp(ts, tz=UTC_TZ).astimezone(NEW_YORK_TZ)
            rows.append({'date': pd.Timestamp(dt_et.date()), 'value': pd.to_numeric(close, errors='coerce')})
        if not rows:
            return empty_df()
        return pd.DataFrame(rows).dropna().drop_duplicates(subset=['date'], keep='last').sort_values('date')
    except Exception:
        return empty_df()
...
def fetch_all_data():
    ...
    # FRED mirrors the same BLS source series while avoiding BLS API quota use.
    fred_map = {
        "us_cpi_nsa": "CPIAUCNS",
        "us_cpi_sa": "CPIAUCSL",
    ...
    for key, fred_id in fred_map.items():
        try:
            data["us_macro"][key] = get_fred_series(fred_id, years=3)
        except Exception as exc:
            data["us_macro"][key] = empty_df()
            errors.append(f"FRED {fred_id}: {exc}")
...
<div class="wrap">
    ...
    <div class="footer">
        Sources: U.S. Treasury · FRED/BLS · Federal Reserve · Bank of Korea · ECOS · Yahoo Finance<br>
