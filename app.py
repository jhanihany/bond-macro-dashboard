...
def get_latest_change(df, value_type):
    ...
    if value_type == "rate":
        ...
        if past.empty or float(past.iloc[-1]["value"]) == 0:
            change = None
        else:
            change = (
                current / float(past.iloc[-1]["value"]) - 1
            ) * 100
    else:
        change = (
...
# ECOS item lookup 1 + US 5 + BOK policy 1 + ECOS series N
...
show_loading(
    "한국은행 ECOS · 산업생산 지표 항목 확인 중"
)
ecos_series["한국 산업생산"]["item"] = (
    find_ecos_item(
        "901Y033",
        ["전산업생산지수"],
    )
